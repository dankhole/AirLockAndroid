...
Decide the app-signing strategy before the first Google Play upload.

For a seamless path between direct APK installs and Google Play installs, enroll
the existing AirLock Goose release key as the **Play app-signing key**. The
direct APK and the APK generated by Google Play will then use the same signing
certificate, so a user can move between distribution paths without uninstalling
the app.

The default Play App Signing option generates a different app-signing key. This
is acceptable for Play-only distribution, but an APK signed with the local key
cannot update a Play-installed build, and a Play-installed build cannot update
the locally signed APK. Testers must uninstall one version before installing
the other.
...
| Key | Use |
| --- | --- |
| App-signing key | Signs direct-distribution APKs and, when enrolled, the APKs Google Play delivers. Keep offline and backed up. |
| Upload key | Signs AABs uploaded to Google Play. Store separately from the app-signing key. |

Confirm that the SHA-256 certificate fingerprint of the chosen app-signing key
matches the key registered in the Android Developer Console. Do not change the
...
app-signing key; losing it prevents normal signed updates outside Play.
...
1. Install AirLock Goose.
2. Grant Usage Access for AirLock Goose.
...
tester list. They can then install and update AirLock Goose through Play Store.
