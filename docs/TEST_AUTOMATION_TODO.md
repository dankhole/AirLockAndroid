...
- [x] Normal polling removes the blocker from Recents, Home, and a directly
  opened unblocked app; Recents also remains blocker-free across the aggregate
  sanity check, the blocker stays above the navigation inset, Android Back
  leaves safely, and the scenario runs under gesture and three-button
  navigation when supported.
